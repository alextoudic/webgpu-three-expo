...
function Box({ position, color }: Props) {
  ...
  return (
    <mesh position={new THREE.Vector3(position[0], position[1], position[2])} ref={ref}>
  ...
  return (
    <>
      ...
      {cylinderData.map((cylinder, index) => (
        <Cylinder key={`cylinder-${index}`} position={cylinder.position} color={cylinder.color} />
      ))}
      {sphereData.map((sphere, index) => (
        <Sphere key={`sphere-${index}`} position={sphere.position} color={sphere.color} />
      ))}
    </>
  );
};
