...
export function createControls() {
  ...
  const functions = {
    shouldClaimTouch(event: GestureResponderEvent) {
      ...
      if (event.nativeEvent.touches.length === 1) {
        const { locationX: x, locationY: y, timestamp: t } = event.nativeEvent.touches[0];
    ...
    handleTouchStartRotate(event: GestureResponderEvent) {
      if (event.nativeEvent.touches.length === 1) {
        ...
        const x =
          0.5 * (event.nativeEvent.touches[0].locationX + event.nativeEvent.touches[1].locationX);
        const y =
          0.5 * (event.nativeEvent.touches[0].locationY + event.nativeEvent.touches[1].locationY);
    ...
    handleTouchStartDolly(event: GestureResponderEvent) {
      ...
      if (event.nativeEvent.touches.length === 2) {
        const dx = event.nativeEvent.touches[0].locationX - event.nativeEvent.touches[1].locationX;
        const dy = event.nativeEvent.touches[0].locationY - event.nativeEvent.touches[1].locationY;
    ...
    pan(deltaX: number, deltaY: number) {
      ...
      const distanceScale = (scope.camera as PerspectiveCamera).isPerspectiveCamera
  ...
  const update = (() => {
    ...
    return () => {
      ...
      const quat = new Quaternion().setFromUnitVectors(scope.camera.up, new Vector3(0, 1, 0));
      ...
      if ((scope.camera as PerspectiveCamera).isPerspectiveCamera) {
        ...
        scope.camera.zoom = Math.max(
          Math.min(scope.camera.zoom / (internals.scale * scope.zoomSpeed), scope.maxZoom),
          scope.minZoom
        );
...
export default function useControls() {
  ...
  return [
    (props: OrbitControlsProps) => <OrbitControls controls={controls} {...props} />,
    controls.events,
  ] as const;
}
